...
export default function AppDetail({ params }: { params: { appId: string } }) {
  ...
  async function fetchAppData() {
    try {
      ...
      const currentApp = allApps.find((a: App) => a.id === params.appId);
      setApp(currentApp || null);

      // Fetch keywords
      const keywordsRes = await fetch(`/api/apps/${params.appId}/keywords`);
      const keywordsData = await keywordsRes.json();
      setKeywords(keywordsData);

      // Fetch recent monitoring results
      const resultsRes = await fetch(`/api/apps/${params.appId}/results`);
      if (resultsRes.ok) {
  ...
  async function addKeyword() {
    ...
    try {
      const res = await fetch(`/api/apps/${params.appId}/keywords`, {
        method: 'POST',
  ...
  async function runMonitoring() {
    setMonitoring(true);
    try {
      const res = await fetch(`/api/apps/${params.appId}/run-monitoring`, {
        method: 'POST',
      });
